...
try {
    $missing = validateRequired($_POST, ['name', 'mobile', 'whatsapp_number', 'course_id', 'admission_date']);
    if (!empty($missing)) {
    ...
    if (!empty($_FILES['photo']['name'])) {
        ...
        if ($photo === null && $_FILES['photo']['error'] !== UPLOAD_ERR_NO_FILE) {
            jsonResponse(false, 'Invalid photo file. Only JPG, PNG, GIF, WEBP are allowed.', [], 422);
        }
    }

    $pdo = db();

    // Verify course exists and fetch its default fee + fee_type
    ...
    $stmt = $pdo->prepare("
        INSERT INTO students (
            name, father_name, mobile, whatsapp_number, parent_contact, gender, dob,
            course_id, admission_date, address, photo,
            qualification, school_college, qualification_year, timing, reference_source,
            fee_mode, discount_percent, final_fee, created_at, updated_at
        )
    ...
    $stmt->execute([
        ...
        $feeMode, $discountPercent, $finalFee,
    ]);

    $newId = $pdo->lastInsertId();

    jsonResponse(true, 'Student added successfully.', ['id' => $newId]);
} catch (Exception $e) {
    jsonResponse(false, 'Failed to add student: ' . $e->getMessage(), [], 500);
}